...
class World:
    ...
    def compute(self, nb_relaxation_iterations: int = 10000):
        ...
        if not self.wires:
            ...
            self._magnetic_field  = BiotSavartEquationSolver.solve(electric_current=self._wires_current)
            self._energy_flux = (1 / mu_0) * (self._electric_field.cross(self._magnetic_field))

    def show_wires_voltage(self, Titre='i'):
        """
        Shows wires' voltage field.
        """
        if self.wires:
            self._wires_voltage.show(title=Titre, x_label='x [m]', y_label = 'y [m]')
        else:
    ...
    def show_electric_field(self, hide_wires: bool = True, Titre='Titre'):
        """
        ...
        if self.wires:
            ...
            electric_field.show(title=Titre, x_label = 'x [m]', y_label = 'y [m]')
        else:
            raise self.EmptyWorldException

    def show_magnetic_field(self, Titre='a'):
        """
        Shows the z-component of the magnetic field.
        """
        if self.wires:
            self._magnetic_field.z.show(title=Titre,x_label='x [m]', y_label = 'y [m]')
        else:
            raise self.EmptyWorldException

    def show_energy_flux(self, Titre='a'):
        """
        Shows the energy flux.
        """
        if self.wires:
            self._energy_flux.show(title=Titre, x_label='x [m]', y_label = 'y [m]')
        else:
            raise self.EmptyWorldException
